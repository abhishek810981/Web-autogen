...
// Define the structure for image entries
...
export async function GET() {
  ...
  // Removed entries for missing images: ai-development.png, visual-builder.png
  // Explicitly type the array even when empty
  ...
  const xml = `<?xml version="1.0" encoding="UTF-8"?>
    <urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9"
      ...
      ${await Promise.all(images.map(async (image) => {
        ...
        const imageFilePath = path.join(process.cwd(), 'public', image.url);
        const metadata = await getImageMetadata(imageFilePath);
